...
if(isset($_POST['registrar'])) {
 ...
 $login = $DBcon->real_escape_string($login);
 $nome = $DBcon->real_escape_string($nome);
 $email = $DBcon->real_escape_string($email);
 $senha = $DBcon->real_escape_string($senha);
 $hashed_password = password_hash($senha, PASSWORD_DEFAULT);
 $nome = utf8_encode($nome);
    
 $check_person = $DBcon->query("SELECT * FROM usuarios WHERE usuario='$login'");
 $count=$check_person->num_rows;
 
if($count==0)
{
  $query = "INSERT INTO usuarios(usuario,nome,senha,email) VALUES('$login','$nome','$hashed_password','$email')";
  if ($DBcon->query($query))
  {
   $query2 = $DBcon->query("SELECT * FROM usuarios WHERE usuario='$login'");
...
{
    $msg2 = "<br><div class='alert alert-danger' role='alert'>
  <strong>Usuário já registrado.</strong>.
</div>"; 
}
}
?>
...
?>  
    ...
    <center><img src="assets/logo.png" width="100"></center>
        <div id="formContent">
          <!-- Tabs Titles -->
          <h2 class="inactive underlineHover"> <a href="acessovereador">Área do Vereador</a> </h2>
          <h2 class="active"> Registrar </h2>
      
          <!-- Login Form -->
          <form role="form" action="" autocomplete="off" method="post">
            <input type="text" id="login" class="fadeIn first" name="login" placeholder="Usuário">
            <input type="text" id="nome" class="fadeIn first" name="nome" placeholder="Nome">
            ...
            <input type="submit" class="fadeIn third" value="Registrar" name="registrar">
          </form>
