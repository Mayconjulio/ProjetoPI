...
if (isset($_POST['novaenquete'])) {
    ...
    $email = $_POST['emailen'];
    $zap = $_POST['zapen'];
    $mensagem = $_POST['sobreen'];

    // Validar os dados, por exemplo, verificar se o email é válido e se o número de telefone está no formato correto.

    $nome = $DBcon->real_escape_string($nome);
    $mensagem = $DBcon->real_escape_string($mensagem);

    $query4 = "INSERT INTO reqcidadao (tipo, nome, email, telefone, mensagem) VALUES ('DN', '$nome', '$email', '$zap', '$mensagem')";
    if ($DBcon->query($query4)) {
        $msg2 = "<br><div class='alert alert-info' role='alert'>
          <strong>Pronto!</strong> Denúncia enviada com sucesso.
        </div>";
    } else {
...
<head>
  ...
  <title>
    Denúncias - Câmara Municipal de <?php echo $nomecam;?>
  </title>
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="card-header">
                <center><h4 class="card-title">Denúncias</h4></center>
              </div>
