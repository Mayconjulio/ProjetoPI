...
if (isset($_POST['novaenquete'])) {
    $titulo = $DBcon->real_escape_string(strip_tags($_POST['tituloen']));
    $sobre = $DBcon->real_escape_string(strip_tags($_POST['sobreen']));

    $query = "INSERT INTO comissoesp (comissao, nomes) VALUES (?, ?)";
    if ($stmt = $DBcon->prepare($query)) {
        $stmt->bind_param("ss", $titulo, $sobre);
        if ($stmt->execute()) {
            // A inserção foi bem-sucedida, não é necessário o redirecionamento
        } else {
...
$(document).on('change', '#enquete2', function(){
    ...
    $.ajax({
        url: "votos.php",
        type: "POST",
        data: { eid: eid, usr: mid, usrname: cnpj },
        dataType: "text",
        success: function (data) {
            if (data !== '') {
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="card-header">
                <center><h4 class="card-title">Comissão Permanente</h4></center>
              </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="form-group" hidden>
                <input type="text" class="form-control" name="lastide" value="<?php echo $lastid; ?>" hidden>
            </div>
            <div class="form-group">
                <label>Nome da Comissão</label>
                <input type="text" class="form-control" placeholder="Nome da Comissão" name="tituloen">
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                ...
                <textarea rows="4" cols="80" class="form-control" placeholder="Membros. Separar cada nome por vírgula" name="sobreen"></textarea>
            </div>
        </div>
    </div>
    <center>
        <button type="submit" class="btn btn-info" name="novaenquete">Confirmar</button>
    </center>
    <br><br>
    <center>
