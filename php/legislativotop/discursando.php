    ...
    while ($usuario = $queryy->fetch_array()) {
        ...
        if ($contagemfinal >= 0) {
            while ($usuariofinal = $queryfinal->fetch_array()) {
                ...
                $output .= '<center><strong><h2 style="color:black;">'.$interval->format($differenceFormat).'</h2></strong></center>';
                $output .= '<input id="trest1" name="trest1" value="'.$interval->format($differenceFormat).'" hidden></input>';
